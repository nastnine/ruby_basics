...
class Interface
  ...
  def menu
    puts(<<~HEREDOC)
      ...
      Нажмите 11, если хотите занять место или объем в вагоне
      Нажмите 12, если хотите вывести список вагонов у поезда
    HEREDOC
  end
  ...
  def show_menu
    ...
    loop do
      ...
      when 10 then show_train_and_station
      when 11 then occupy
      when 12 then show_wagons
      else
        puts 'Error!'
      end
    end
  ...
  def add_wagon
    ...
    if selected_train.is_a?(PassengerTrain)
      ...
      seats = gets.to_i
      selected_train.add wagon(PassengerWagon.new(seats))
  ...
  def occupy
    show_array(trains)
    puts 'Выберете поезд, чтобы занять место или объем в вагоне'
    selected_train = select_from_array(trains)
    retern if selected_train.nil?
